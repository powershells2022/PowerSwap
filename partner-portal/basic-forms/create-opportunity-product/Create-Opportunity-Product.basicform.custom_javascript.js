$(document).ready(function () {
    ...
    if ($('#priceperunit').length || $('#quantity').length)
    {
        isRefreshed = true;
    }
    
    CreateOpportunityProductTypeValidators();
 VerifyOpportunityProductTypeValidators(isRefreshed);
});
...
function VerifyOpportunityProductTypeValidators(isRefreshed) {
 ...
 if (tableData) {
  if ($('#isproductoverridden_1').is(':checked')) {
   ...
   if ($('#ValidationSummaryEntityFormControl_EntityFormView').length && !isRefreshed) {
